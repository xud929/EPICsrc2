...
void beam::set_longitudinal_slices2() {
	...
	slice_npar.clear();
    slice_npar.resize(zslice,0.0);
	//slice_xc.clear();
    //slice_xc.resize(zslice,0.0);
	//slice_yc.clear();
    //slice_yc.resize(zslice,0.0);
	//slice_xrms.clear();
    //slice_xrms.resize(zslice,0.0);
	//slice_yrms.clear();
    //slice_yrms.resize(zslice,0.0);
    ...
    //double z_bin=(zmax-zmin+1.0e-3)/zslice;
	...
	for(unsigned i=0;i<zslice;++i){
	  par_inslice[i].resize(end_Ind-start_Ind);
	}
	vector<unsigned long> par_inslice_size(zslice,0);
    ...
    for (unsigned long i=start_Ind;i<end_Ind;i++){
        if (inslice[i]>=0) {
            ...
            double zext;
            if (zoff > 0) {
                zext = zoff + (marco_size - z_bin) / 2.0;
				if(zext<=0 || inslice[i]==zslice-1){
				  partition[i]=1.0;
				  adslice[i]=-1;
				}else{
				  partition[i]=1.0-zext*zext*2.0/marco_size/marco_size;
				  adslice[i]=inslice[i]+1;
				}
            } else {
                zext = zoff + (z_bin-marco_size) / 2.0;
				if(zext>=0 || inslice[i]==0){
				  partition[i]=1.0;
				  adslice[i]=-1;
				}else{
				  partition[i]=1.0-zext*zext*2.0/marco_size/marco_size;
			...
			/*
			par_inslice[inslice[i]].push_back(i);
			if(adslice[i]>=0 && inslice[i]!=adslice[i])
			  par_inslice[adslice[i]].push_back(i);
			  */
			//#pragma omp atomic read
			auto &current=par_inslice_size[inslice[i]];
			par_inslice[inslice[i]][current++]=i;
			if(adslice[i]>=0){
			  //#pragma omp atomic read
			  auto &current=par_inslice_size[adslice[i]];
			  par_inslice[adslice[i]][current++]=i;
			  //#pragma omp atomic
	...
	for(unsigned i=0;i<zslice;++i)
	  par_inslice[i].resize(par_inslice_size[i]);

	/*
	par_inslice.clear();
	par_inslice.resize(zslice);
	for(unsigned i=0;i<zslice;++i){
	  if(!par_inslice_set[i].empty()){
...
/************************************************************************************************/
double beambeam_pass2(beam &beamL, beam &beamR){
  int rank, size;
  MPI_Comm_rank(MPI_COMM_WORLD, &rank);

  double totallum=0.0;
  map<double,vector<pair<int, int>>> mco; //map of collision order

  //double zL=beamL.slice_center[sL], zR=beamR.slice_center[sR];
  const double & slice_width_L=beamL.z_bin,  & slice_width_R=beamR.z_bin;
  /*
  if(rank==0){
	std::cout<<slice_width_L<<"\t"<<slice_width_R<<std::endl;
  }
  */
  ...
  do{
	...
	for(iL=iL_start;iL>iL_end;--iL){
	  iR=index-iL;
	  mco[0.5*((nL-1-iL)*slice_width_L+(nR-1-iR)*slice_width_R)].push_back(std::make_pair(iL,iR));
	}
  ...
  for(const auto & current : mco){
	auto nC=current.second.size();
	vector<vector<vector<double>>> coordL(nC), coordR(nC);
	for(int i=0;i<nC;++i){
	  int iL=current.second[i].first;
	  int iR=current.second[i].second;

	  totallum+=beambeam_slicepass(beamL,iL,coordL[i],beamR,iR,coordR[i]);
	...
	// delta_x=g*(x1-x0)+(1-g)*(x2-x0)
	for(int i=0;i<nC;++i){
	  int iL=current.second[i].first;
	  #pragma omp parallel for
	  for(unsigned k=0;k<beamL.par_inslice[iL].size();++k){
		unsigned ip=beamL.par_inslice[iL][k];
		double partition=(iL==beamL.adslice[ip]?1.0-beamL.partition[ip]:beamL.partition[ip]);
		xL[ip]+=partition*(coordL[i][0][k]-beamL.x_[ip]);
		pxL[ip]+=partition*(coordL[i][1][k]-beamL.px_[ip]);
		yL[ip]+=partition*(coordL[i][2][k]-beamL.y_[ip]);
		pyL[ip]+=partition*(coordL[i][3][k]-beamL.py_[ip]);
	  }
	  ...
	  for(unsigned k=0;k<beamR.par_inslice[iR].size();++k){
		unsigned ip=beamR.par_inslice[iR][k];
		double partition=(iR==beamR.adslice[ip]?1.0-beamR.partition[ip]:beamR.partition[ip]);
		xR[ip]+=partition*(coordR[i][0][k]-beamR.x_[ip]);
		pxR[ip]+=partition*(coordR[i][1][k]-beamR.px_[ip]);
		yR[ip]+=partition*(coordR[i][2][k]-beamR.y_[ip]);
		pyR[ip]+=partition*(coordR[i][3][k]-beamR.py_[ip]);
	  }
	}
	// x=x0+delta_x
	for(int i=0;i<nC;++i){
...
double beambeam_slicepass(beam &beamL, unsigned sL, vector<vector<double>> &coordL, beam &beamR, unsigned sR, vector<vector<double>> &coordR){
  ...
  for(unsigned i=0;i<partL.size();++i){
	...
	//auto & ip=partL[i];
	//double partition=(sL==beamL.adslice[ip]?1.0-beamL.partition[ip]:beamL.partition[ip]);
	double partition=(sL==beamL.adslice[partL[i]]?1.0-beamL.partition[partL[i]]:beamL.partition[partL[i]]);
  ...
  for(unsigned i=0;i<partR.size();++i){
	...
	//auto & ip=partR[i];
	//double partition=(sR==beamR.adslice[ip]?1.0-beamR.partition[ip]:beamR.partition[ip]);
	double partition=(sR==beamR.adslice[partR[i]]?1.0-beamR.partition[partR[i]]:beamR.partition[partR[i]]);
	...
	if(beamR.inslice[partR[i]]==sR){
	  double partition=beamR.partition[partR[i]];
	  momR[0]+=partition*coordR[0][i];
	  momR[1]+=partition*coordR[0][i]*coordR[0][i];
	  momR[2]+=partition*coordR[2][i];
	  momR[3]+=partition*coordR[2][i]*coordR[2][i];
	...
	if(beamR.adslice[partR[i]]==sR){
	  double partition=1.0-beamR.partition[partR[i]];
	  momR[0]+=partition*coordR[0][i];
	  momR[1]+=partition*coordR[0][i]*coordR[0][i];
	  momR[2]+=partition*coordR[2][i];
	  momR[3]+=partition*coordR[2][i]*coordR[2][i];
  ...
  /*
  double flumL;
  if(momR[1]!=0 && momR[3]!=0)
	flumL=macL*numR/twopi/momR[1]/momR[3];
  else
	flumL=0.0;
  */
  #pragma omp parallel for simd
...
vector<double> beam::CalMoments(){
  vector<double> moments(10,0.0);
  ...
  for (unsigned long i=start_Ind;i<end_Ind;i++){
	if(inslice[i]<0)
	  continue;
	moments[0]+=x_[i]; //mean
	moments[1]+=x_[i]*x_[i]; //var
	moments[2]+=px_[i];
	moments[3]+=px_[i]*px_[i];
	moments[4]+=x_[i]*px_[i];

	moments[5]+=y_[i];
	moments[6]+=y_[i]*y_[i];
	moments[7]+=py_[i];
	moments[8]+=py_[i]*py_[i];
	moments[9]+=y_[i]*py_[i];
  }

  MPI_Allreduce(MPI_IN_PLACE, moments.data(),10, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  for(auto & i : moments){
	i/=static_cast<double>(n_live);
  }

  moments[1]-=moments[0]*moments[0];
  moments[3]-=moments[2]*moments[2];
  moments[4]-=moments[0]*moments[2];

  moments[6]-=moments[5]*moments[5];
  moments[8]-=moments[7]*moments[7];
  moments[9]-=moments[5]*moments[7];
